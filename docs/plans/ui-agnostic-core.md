...
**Status:** ✅ all tasks complete (T-1 through T-9). Follow-up work tracked in [`ui-agnostic-polish.md`](./ui-agnostic-polish.md).
...
### T-6 — React conformance tests over shared scenarios  ✅ complete
...
- **T-9** ✅ complete — Documentation refresh across all 7 READMEs. Root README leads with "Pick your binding" matrix (React / vanilla / Web Component rows). `packages/core/README.md` has `createBabulfish` quick-start and `@babulfish/core/testing` pointer. `packages/react/README.md` covers `TranslatorProvider`, `useTranslator`, components, and CSS import. `packages/styles/README.md` documents all custom properties (added missing `--babulfish-muted` declaration to CSS). `packages/demo/README.md` created (was absent). `packages/demo-vanilla/README.md` and `packages/demo-webcomponent/README.md` expanded with runnable instructions and cross-links. All cross-links resolve. No decision changes vs. design doc.
